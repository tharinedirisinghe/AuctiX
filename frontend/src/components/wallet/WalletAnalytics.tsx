...
const WalletAnalytics: React.FC<WalletAnalyticsProps> = ({
  ...
  const analyticsData = useMemo(() => {
    ...
    const balanceHistory = sortedTransactions.map((transaction, index) => {
      ...
      if (transaction.status === 'CREDITED' || transaction.status === 'UNFREEZED') {
        runningBalance += amount;
      } else if (transaction.status === 'DEBITED' || transaction.status === 'COMPLETED') {
        runningBalance -= amount;
      }
  ...
  const getTypeColor = (type: string) => COLORS[type as keyof typeof COLORS] || '#6b7280';

  if (!transactions || transactions.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Wallet Analytics</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-6">Wallet Analytics</h3>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-lg font-bold text-green-700">
                  LKR {analyticsData.totalCredits.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-lg font-bold text-red-700">
                  LKR {analyticsData.totalDebits.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-lg font-bold text-blue-700">
                  LKR {analyticsData.avgTransactionAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Transaction Types */}
          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="text-md font-medium text-gray-900 mb-4">Transaction Distribution</h4>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                      ...
                      return (
                        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
                          ...
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Count:</span> {data.value} transactions
                          </p>
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Amount:</span> LKR {data.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                          </p>
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Avg per transaction:</span> LKR {(data.amount / data.value).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                          </p>
                        </div>
                      );
                    }
          ...
          {/* Weekly Trend */}
          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="text-md font-medium text-gray-900 mb-4">Weekly Activity</h4>
            <ResponsiveContainer width="100%" height={250}>
          ...
          {/* Monthly Spending */}
          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="text-md font-medium text-gray-900 mb-4">Monthly Overview</h4>
            <ResponsiveContainer width="100%" height={250}>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Financial Insights */}
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            <h4 className="text-md font-medium text-blue-900 mb-3">Financial Analysis</h4>
            <div className="text-sm text-blue-800 space-y-2">
              <div className="flex justify-between">
                <span>Net Cash Flow:</span>
                <span className={`font-semibold ${(analyticsData.totalCredits - analyticsData.totalDebits) >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                  LKR {(analyticsData.totalCredits - analyticsData.totalDebits).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </span>
              </div>
              <div className="flex justify-between">
                <span>Balance Utilization:</span>
                <span className="font-semibold">
                  {((currentBalance / (currentBalance + frozenBalance)) * 100).toFixed(1)}% available
                </span>
              </div>
              <div className="flex justify-between">
                <span>Credit to Debit Ratio:</span>
                <span className="font-semibold">
                  {analyticsData.totalDebits > 0 ? (analyticsData.totalCredits / analyticsData.totalDebits).toFixed(2) : '∞'} : 1
                </span>
              </div>
              {frozenBalance > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-amber-700">
                    LKR {frozenBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              )}
            </div>
          ...
          {/* Activity Insights */}
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            <h4 className="text-md font-medium text-green-900 mb-3">Activity Insights</h4>
            <div className="text-sm text-green-800 space-y-2">
                ...
                <span className="font-semibold">
                  LKR {(() => {
                    const daysSinceFirst = transactions.length > 0 ? 
                      Math.max(1, Math.ceil((new Date().getTime() - new Date(transactions[0].transactionDate).getTime()) / (1000 * 60 * 60 * 24))) : 1;
                    return (analyticsData.totalDebits / daysSinceFirst).toLocaleString('en-US', { minimumFractionDigits: 2 });
                  })()}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {(() => {
                    const maxType = analyticsData.transactionTypes.reduce((max, type) => 
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {(() => {
                    const weeklyTotal = analyticsData.weeklyTrend.reduce((sum, day) => sum + day.credits + day.debits, 0);
        ...
        <div className="mt-4 bg-purple-50 rounded-lg p-4 border border-purple-200">
          <h4 className="text-md font-medium text-purple-900 mb-3">Spending Patterns</h4>
            ...
            <div className="text-purple-800">
              ...
              <div className="text-lg font-bold">
                LKR {(() => {
                  ...
                  return Number(maxTransaction.amount).toLocaleString('en-US', { minimumFractionDigits: 2 });
                })()}
              </div>
            </div>
            <div className="text-purple-800">
              ...
              <div className="text-lg font-bold">
                LKR {(() => {
                  const monthsActive = Math.max(1, new Set(transactions.map(t => 
                    new Date(t.transactionDate).toISOString().slice(0, 7)
                  )).size);
                  return (analyticsData.totalDebits / monthsActive).toLocaleString('en-US', { minimumFractionDigits: 2 });
                })()}
              </div>
            </div>
            <div className="text-purple-800">
              ...
              <div className="text-lg font-bold">
                {(() => {
                  ...
                  const efficiency = totalBalance > 0 ? ((currentBalance / totalBalance) * 100) : 0;
        ...
        <div className="mt-4 bg-amber-50 rounded-lg p-4 border border-amber-200">
          <h4 className="text-md font-medium text-amber-900 mb-3">Trend Analysis</h4>
          <div className="text-sm text-amber-800 space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-semibold ${(analyticsData.totalCredits - analyticsData.totalDebits) >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                {(() => {
                  const growth = ((analyticsData.totalCredits - analyticsData.totalDebits) / Math.max(1, analyticsData.totalCredits)) * 100;
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {(() => {
                  const monthlyAmounts = analyticsData.monthlySpending.map(m => m.debits);
                  ...
                  const consistency = variance < avg * 0.5 ? 'High' : variance < avg ? 'Medium' : 'Low';
                  return consistency;
                })()}
              </span>
            </div>
